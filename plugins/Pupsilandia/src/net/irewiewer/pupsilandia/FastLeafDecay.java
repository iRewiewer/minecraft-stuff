...
public class FastLeafDecay implements Listener
{
	...
	public FastLeafDecay(final Main pl) { this.plugin = pl; }
    ...
    static final List<Block> scheduledBlocks = new ArrayList<>();
        ...
        for (BlockFace neighborFace: NEIGHBORS)
        {
            ...
            if (oneByOne)
            {
                if (scheduledBlocks.isEmpty())
                	Bukkit.getServer().getScheduler().runTaskLater(plugin, this::decayOne, delay);
                scheduledBlocks.add(block);
            } else
            	Bukkit.getServer().getScheduler().runTaskLater(plugin, () -> decay(block), delay);
